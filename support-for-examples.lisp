...
(in-package #:terms-user)
#+debug-action-theory
(declaim (optimize (debug 3) (space 1) (speed 0) (compilation-speed 0)))

(defvar *terms-examples*
  (make-hash-table))

(defun delete-example (name)
  (remhash name *terms-examples*))

(defun add-example (name example)
  (setf (gethash name *terms-examples*) example))

(defun find-example (name)
  (gethash name *terms-examples* nil))

(defclass terms-example ()
  ((name :accessor name :initarg :name
   ...
   (max-solution-depth :accessor max-solution-depth 
                       ...
                       :initform terms::*default-max-solution-depth*)
...
(defmethod print-object ((self terms-example) stream)
  (print-unreadable-object (self stream :type t)
    (format stream "~A" (name self))))

(defmethod make-load-form ((self terms-example) &optional environment)
...
(defmethod initialize-instance :after ((self terms-example) &key name)
...
(defun record-variable-values-for-test ()
  (iterate (for var in *terms-test-variables*)
    ...
    (let ((interpreter (default-interpreter)))
      ...
      (let ((test-case (if record-trace-p
                           (make-instance 'example-test-case
                             ...
                             :execution-mode execution-mode)
                           nil))
            (terms::*default-max-solution-depth* (max-solution-depth example)))
...
(defun all-examples ()
  (let ((keys (copy-list (hash-table-keys *terms-examples*))))
